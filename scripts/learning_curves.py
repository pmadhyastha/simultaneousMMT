...
def metric_subplot(ax, x, y, name, color):
...
def extract_metrics(path):
    ...
    for line in ''.join(metric_lines).split('\n'):
...
def plot_metrics(title, metrics):
    xs = range(len(metrics['BLEU']))
    h, w = (2, 3)
    f, axes = plt.subplots(h, w)
    plt.suptitle(title, size=16)

    score_it = iter(metrics.items())
    colors_it = iter(sns.color_palette('pastel', n_colors=h * w))
    # generate subplot for each name, data pair in scores
    for y in range(h):
        for x in range(w):
            name, data = next(score_it)
            metric_subplot(axes[y, x], xs, data, name, next(colors_it))

    f.tight_layout()
    plt.subplots_adjust(top=0.88)
    # plt.show()
    plt.savefig(f'figs/{"_".join(title.split(" "))}.png')


def extract_and_plot(title, path):
    metrics = extract_metrics(path)
    plot_metrics(title, metrics)


def plot_unimodal_length_coeff():
    # plot unimodal length_coeff experiments
    out_dir = '../experiments/rl/'
    prefix = 'en-fr-unimodal-rl-length-coeff-'
    suffix = '.out'
    fns = [x for x in os.listdir(out_dir) if prefix in x]

    for fn in fns:
        hyperparam = float(fn[len(prefix): -len(suffix)])
        title = f'Unimodal simnmt RL, length_coeff = {hyperparam}'
        path = os.path.join(out_dir, fn)
        extract_and_plot(title, path)


def plot_multimodal_length_coeff():
    # plot multimodal length_coeff experiments
    out_dir = '../experiments/rl/'
    prefix = 'en-fr-multimodal-rl-length-coeff-'
    suffix = '.out'
    fns = [x for x in os.listdir(out_dir) if prefix in x]

    for fn in fns:
        hyperparam = float(fn[len(prefix): -len(suffix)])
        title = f'Multimodal simnmt RL, length_coeff = {hyperparam}'
        path = os.path.join(out_dir, fn)
        extract_and_plot(title, path)


def plot_multimodal_agent_side():
    path = "../experiments/rl/en-fr-cgru-simmmtrl-multimodal-agent-side-slurm-7646.out"
    title = f'Agent side multimodal RL'
    extract_and_plot(title, path)

def model_coeff_mapping():
    print('model mapping to length_coeff')
    out_dir = '../experiments/rl/'
    prefix = 'en-fr-unimodal-rl-length-coeff-'
    suffix = '.out'
    fns = [x for x in os.listdir(out_dir) if prefix in x]
    for fn in fns:
        print(f'param = {fn[len(prefix):-len(suffix)]}')
        path = os.path.join(out_dir, fn)
        with open(path) as file:
            model_name_pattern = re.compile(r'exp_id:(simrl.*)')
            model_name = re.search(model_name_pattern, file.read())
            print(model_name.group())

if __name__ == "__main__":
    plot_unimodal_length_coeff()
    plot_multimodal_agent_side()
    plot_multimodal_length_coeff()
