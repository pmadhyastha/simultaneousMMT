...
def metric_subplot(ax, x, y, name, color):
    subplot = sns.lineplot(x=x, y=y, ax=ax, color=color)
    if name == "SDIFF":
...
Validation 31 -> BLEU = 46.51, 70.3/52.2/40.5/31.7 (BP=0.998, ratio=0.998, hyp_len=14349, ref_len=14382)
    ...
    for fn in fns:
        ...
        title = f'Agent side multimodal simnmt RL, length_coeff = {hyperparam}'
        path = os.path.join(out_dir, fn)
        method_name(title, path)
